...
export const EvistaAPI = {
   ...
   */
  hotel: {
    getTransactions: async (slug, { month, page = 1, perPage = 20 } = {}) => {
      const params = new URLSearchParams({ page, per_page: perPage });
      if (month) params.set('month', month);
      return localApiRequest(`/api/hotel/${slug}/transactions?${params.toString()}`);
    },
    getPayouts: async (slug) => {
      return localApiRequest(`/api/hotel/${slug}/payouts`);
    },
  },
  /**
  ...
  partner: {
    login: async (email, password) => {
      return apiRequest('/api/v4/auth/login', {
        method: 'POST',
...
export const BookingFlow = {
  ...
  /**
   * Booking Flow Helper
   * Orchestrates the complex booking process
